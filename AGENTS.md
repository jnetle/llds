...
<NEXT_PUBLIC_IMG_BASE>/
  projects/<asset-key>/<descriptive-slug>.jpg        # every file named in the record's `gallery`, in any order
                                                     # asset keys = PROJECT_META assetKey values in lib/projects.ts
...
- **Project photos are switched on per project, not globally.** Upload the shoot to `projects/<asset-key>/` and author it as the record's `gallery` — one `{ file, alt, aspect? }` entry per photograph, in display order. That array _is_ the switch: present and non-empty means the project has real photography, so there is no `assetsReady` flag to forget. Only the folder is derived (`assetKey`); the leaf is authored, so no URL is ever pasted into the data. **Order lives in the array, never in the file names** — renaming objects on R2 to reorder would leave every renamed URL serving a stale optimized derivative for `minimumCacheTTL` (31 days). **`cover` and `hero` are two different pickings, and neither implies the other.** `cover` is the tile on /projects, the home hero and the strip — it has to read at thumbnail size, against other projects, and say which project this is. `hero` is what the detail page opens on, at full bleed or half a viewport, with nothing competing and the title beside it, so it can be the quieter or wider frame that would vanish in a tile. Each names one file in the same folder, normally one already in `gallery`, which costs no second object and inherits that plate's alt text. **`hero` has no fallback — it is required alongside `gallery`**, so `tsc` fails a shot record that does not say what its page opens on (see `ShotRecord`). It used to default to the first plate, which read as a choice while being an accident: four of the eight published records had re-picked `cover` away from plate one, and their pages went on opening on plate one because nothing said otherwise. A fallback cannot be reviewed. Naming the same file in both fields is fine and costs no second object — it just has to be stated. `cover` does still default to the first plate. The field is `hero`, not `banner`, because `heroTemplate` is the sibling that chooses its _layout_ and `banner` is one of that field's values. Route slugs can differ; storage paths stay stable via `assetKey`. **Until a record has a `gallery` the site does not publish it at all** — `PROJECTS` in `lib/projects.ts` is `ALL_PROJECTS` filtered by `hasRealAssets`, so an unshot record is absent from the index, the home hero and strip, prev/next, the sitemap and `generateStaticParams`, and `getProject` does not resolve its slug, which 404s the detail route. Authoring the `gallery` is therefore what publishes a project; read `ALL_PROJECTS` only for data guards and tooling that must see unpublished work. The placeholder pool still backs those hidden records so they render while being authored — when every record has a `gallery`, delete `PLACEHOLDER_ASSETS`, the `hasRealAssets` filter and `ALL_PROJECTS`, and drop `images.unsplash.com` from `next.config.ts`.
...
- `components/` — feature components (`Header`, `Footer`, `HeroGrid`, `GridCell`, `ProjectsGrid`, `ProjectDetail`, `StatementSection`, `ProjectStrip`, `Wordmark`, `HomeShell`); route-scoped ones sit in a subfolder (`components/press/`, `components/project/`, `components/testimonials/`)
- **The project gallery has swappable templates, chosen by data.** `galleryTemplate` on the record in `lib/projects.ts` picks one of `components/project/Gallery*.tsx`, and `ProjectDetail` only dispatches — it holds the selected index and the `Built by` line, which every template shares. `plates` (the default) crops each photograph to a full-bleed band and suits a shoot cut to a handful of frames; `masonry` is an Unsplash-style column grid that publishes every frame uncropped and suits a full shoot. **They differ in interaction, not just layout:** a plate is a button that promotes itself into the hero, while masonry tiles are inert figures — the hero sits thousands of pixels above the lower columns there, so the same click would land with nothing visibly happening. A template that cannot make an affordance pay off should not render one. **Adding a template is a new file plus a branch, never an edit to an existing one** — a layout that has shipped is the one thing a live project's page is depending on.
- **TEMPORARY — `?files=1` stamps every photograph on a project page with its file name.** `components/project/FileBadge.tsx`, on the masonry tiles and on the hero of both detail templates, so a shoot can be reviewed by name ("drop 6664", "move this up") against the live page; each name is selectable and has a copy button. Off without the query parameter, so it can ride through a deploy unseen. It reads `window.location` rather than `useSearchParams` because every detail page is statically prerendered and a static page calling `useSearchParams` from a Client Component fails the build unless each call site is wrapped in Suspense — reading it in the component leaves the route's rendering strategy untouched. To remove the aid: delete that file and its three call sites.
- **The detail page's gallery renders the shoot minus the hero frame.** `hero` normally names a plate — that is what lets it inherit the plate's alt text and cost no second object on R2 — so rendering the array as authored showed the reader the same photograph twice, once as the banner and again a scroll later as the first tile. `ProjectDetail` filters it out by `src`; the record still holds the whole shoot and `lib/schema.ts` still publishes every frame. **This shifts masonry run parity by one.** Each record's notes tune the `feature` positions so every run holds an even number of plates and its two columns end level, and the dropped tile is always in the first run — so a run documented as even now renders odd, and a run of uniform 3/4 plates that is one short ends ~1.33 column widths ragged. Re-tune by moving a `feature`, not by re-adding the duplicate. A `hero` naming a frame the `gallery` does not publish avoids both problems — which is the shape `PLACEHOLDER_ASSETS` now uses: it names `hero` as its own role alongside `cover` and `gallery`, pointing at a pool photograph that is not in its own entry, so a placeholder record keeps all three tiles instead of rendering two numbered "view 2, view 3". **The banner's alt text is load-bearing after the de-dupe**: the plate that described that frame is no longer on the page, so `hero` inherits the plate's authored `alt` and a derived one there is a regression, not a floor. `deriveAlt` has a `'hero'` role for the same reason — a banner must never be numbered into a gallery sequence it is not part of. Both are asserted in `lib/__tests__/projects.test.ts`.
- `masonry` is the only consumer of a plate's `aspect` anywhere, because it is the only layout that does not crop: it has to reserve each tile's true box before the file loads, or the column reflows mid-load. Set it only on the frames that differ from the 3/4 default, and read the value off `scripts/compress-images.mjs`, which prints each output's dimensions for exactly this reason.
- **A photograph can credit one object in it, the way a magazine credits what the model is wearing.** A plate's optional `credit` (`{ piece, note }` in `lib/projects.ts`) names a slug in `lib/pieces.ts` and the sentence the caption prints; `components/pieces/PhotoCredit.tsx` renders it as a `<figcaption>` beneath the frame, with the maker's name as a link to `/pieces/<slug>`, where a visitor can request the piece. **Nothing renders unless a plate names a credit**, so the feature is inert on every project until one is authored. Three rules, all from the layouts rather than from taste:
  - **Never credit the frame `hero` names.** `ProjectDetail` drops that frame from the rendered gallery, so the caption would be authored and invisible. `scripts/check-content.mjs` fails the build on it.
  - **In a `masonry` project, prefer a `feature` frame.** `packColumns` models a tile as exactly `1 / aspect` column-widths tall and deliberately excludes anything measured in pixels; a caption is pixel-height _and_ wrap-dependent, so it cannot join the model. A feature sits between packed runs, where a caption costs the packing nothing. Beneath an ordinary tile the credit still renders and leaves that column roughly one caption taller than its neighbour.
  - **In `plates`, the caption is a sibling of the band's `<button>`, never a child** — a link inside a button is invalid HTML and two overlapping click targets. That is also why the button now carries `display: block; width: 100%`: it stopped being a direct grid item when the `<figure>` came between them, and an inline-block button shrink-wraps a child that sets only a height.
  - A piece carries **no image fields and no R2 objects of its own** — `framesCreditingPiece` in `lib/projects.ts` borrows the project frames that credit it, which is both cheaper and the better photograph. The import runs one way, `projects` → `pieces`, which is what keeps that reverse index out of a cycle. **Do not add a `price` field**: trade pricing is confidential and many brands enforce MAP, so `lib/schema.ts` emits `Product` with a `brand` and deliberately no `offers`.
- **The detail page's hero is swappable the same way, on its own axis.** `heroTemplate` picks one of `components/project/Hero*.tsx`; the two axes are independent, so any hero pairs with any gallery. A hero template owns the photograph **and** the title block, because `split` is exactly the arrangement where those stop being stacked siblings — extracting only the image would leave the copy stranded below it. `banner` (the default) is the original: one frame cropped to the full viewport width, title beneath. `split` puts the frame on the left at its own `aspect` and the title block beside it, the way the About hero reads. **Pick from the shape of `gallery[0]`**, which is what the hero opens on: `banner` crops to a wide band, so a portrait frame loses its top and bottom to it. Both templates take the identical `ProjectHeroProps` (`components/project/heroProps.ts`) so the branch in `ProjectDetail` stays dumb, and the meta line is composed upstream so the two cannot word it differently. The same rule as the galleries applies — **a new template is a new file plus a branch, never an edit to one that has shipped.**
- `split` is built like the About hero, and the mechanism is `min-h` plus grid's default `stretch`: the row is at least 88vh and grows with the taller column, and both items stretch to it, so the photograph fills its half outright — left edge to column edge, top to bottom — with `objectFit: cover` taking up the difference. It has no height or `aspect-ratio` of its own to keep in step. **Do not add `items-center`**: that shrinks each item to its content and strands the frame in the middle of its half, which is what an aspect-sized box did before. What makes the template kind to a portrait is the shape of the box, not an absence of cropping — half a viewport at full height is itself roughly a portrait, so a 3/4 frame loses a little top and bottom here against the two-thirds it loses to a full-width band. The copy's gutter is symmetric from 601px up and holds the measure itself (`max(8vw, half the slack over 620px)`) rather than capping it with a `max-width`, so the gap to the photograph and the gap to the page edge are one value and cannot drift.
- **Scroll-scrubbed sections** (`components/press/MagazineReader.tsx` and the home page's `components/ProjectStrip.tsx`) follow one pattern: a tall track with a `position: sticky; top: 0; height: 100svh` stage, and `hooks/useTrackProgress.ts` reporting 0→1 across the pinned range. That hook reads the track's rect **every frame rather than caching it at mount** — the strip sits below the cover panel, which unmounts and slides the whole track ~100svh up the page, and cached geometry put the pinned range where the reader had already scrolled past, so the strip never moved at all. A `ResizeObserver` is not a substitute: `html` is viewport-height here, so observing the document element never fires. That hook takes a **callback**, not state, on purpose — `useScrollY` re-renders its whole consuming subtree once per scroll frame, so anything driving more than a couple of properties should write to DOM nodes through refs and keep React state for discrete values only (the reader re-renders ~5 times per pass, when the page index changes). Reduced-motion and no-JS fallbacks belong in CSS, not a `usePrefersReducedMotion()` / `useCompact()` branch: both initialise `false`, so a JS branch renders the animated layout first and swaps after hydrate. See the `@media (prefers-reduced-motion: reduce), (scripting: none)` blocks under `.mag-*` and `.strip-*` in `globals.css` — the strip's turns the pinned pass back into an ordinary horizontal scroller, which reaches the last tile by other means.
...
## Inquiry form → ClickUp + confirmation email

`/inquire` and `/pieces/<slug>` are the only routes with a server round-trip, and `app/inquire/actions.ts` and `app/pieces/actions.ts` are the only `'use server'` files in the repo. An inquiry submission becomes **one ClickUp task with a PDF attached**, and then **one confirmation email to the visitor**.

The piece request form is the same shape with **both the PDF and the confirmation email removed** — it becomes one ClickUp task and nothing else, in its own list (`CLICKUP_PIECES_LIST_ID`, falling back to `CLICKUP_LIST_ID` so an unset value misfiles a lead rather than losing one). It reuses `lib/clickup.ts` unchanged. Dropping the email also drops the only reason it had to rate-limit, so **nothing throttles it**: the visitor's acknowledgement is the success panel, which reads their address back to them because nothing else will tell them they mistyped it. `app/pieces/actions.ts` documents the trade and names the commit to recover the email from. Everything below describes the inquiry. Needs `CLICKUP_API_TOKEN` (a `pk_…` personal token, sent bare in `Authorization` — no `Bearer`) and `CLICKUP_LIST_ID`; the email additionally needs `RESEND_API_KEY` and `INQUIRY_FROM_EMAIL`.
...
**The confirmation email** goes out last, in `sendConfirmation` at the bottom of `app/inquire/actions.ts`. The ordering is the same kind of rule as the PDF's: the mail tells the visitor their inquiry arrived, so it must not be sent before the task exists — and once the task exists the lead is safe, so **a send failure is logged and nothing more**, exactly like the attachment upload. Missing config is a _skip_, not an error, unlike the ClickUp guard above it: unset is the correct state in dev and on Preview.

- It is scheduled with **`after()` from `next/server`**, not awaited inline, so the visitor's success panel is never held open for a Resend round-trip that can run to the full 10s timeout. Use `after` rather than a bare un-awaited promise — the serverless function can be frozen the moment the response is sent, which would drop the send silently.
- **The send is throttled; the ClickUp write is not.** `to:` is an address the submitter chose, so an unthrottled form lets anyone make the verified studio domain mail a stranger, and burn the 100/day ceiling so real confirmations stop. `mayEmail` in `app/inquire/actions.ts` caps sends per IP (3/hour) and per recipient (2/hour) via `lib/rateLimit.ts` — two dimensions because they stop different attacks. Deliberately nothing throttles the task creation: **losing a real lead is the worst outcome this form has**, so a throttled resubmission still reaches the studio and only skips the second email.
- `lib/rateLimit.ts` is a rolling window in module memory, so each serverless instance counts separately and the real ceiling is the limit times the warm-instance count. That hole is accepted on purpose — it stops naive scripts without a Redis dependency or a per-request network hop, on a form that sees a handful of submissions a week. Swap it for `@upstash/ratelimit` if the threat ever becomes distributed; `allow()` has the same shape.
- **`sendConfirmation` wraps its whole body in try/catch, and that is load-bearing.** `sendEmail` guards only its own `fetch`; rendering the bodies is local work that can throw on its own — `absoluteUrl()` raises `TypeError: Invalid URL` if `NEXT_PUBLIC_SITE_URL` is set without a scheme. Without the catch, that throw would escape a submission whose ClickUp task already exists, and the visitor's retry would file the lead twice.

- `lib/email.ts` is a plain `fetch` to Resend's `POST /emails`, modelled on `lib/clickup.ts` — **no SDK, no new dependency**. Note the contrast in auth: Resend wants `Bearer`, ClickUp wants the token bare.
- `lib/inquiryEmail.ts` builds the subject, HTML, and plain-text bodies. It is the **third** renderer in the repo with no CSSOM (after the PDF and `lib/og.tsx`), so the same two rules apply: brand hexes written literally, and inline `style` attributes only — no `<style>` block, no classes, no Tailwind. It also does not load Cormorant; a webfont in email silently falls back, so it uses a web-safe serif stack.
- **It escapes what the PDF and markdown do not.** `toFullMarkdown` leaves answers unescaped because ClickUp renders them in a trusted context; the email interpolates the visitor's own name into HTML someone else's client will parse, so `lib/inquiryEmail.ts` has its own `esc()`. Don't reuse the markdown builders for a mail body.
- Both `html` and `text` are always sent. The plain-text alternative is deliverability, not politeness, and matters most while the sending domain has no reputation.
- Resend will not deliver to arbitrary recipients until the studio domain is DNS-verified (SPF + DKIM). The `onboarding@resend.dev` sandbox sender only reaches your own account address, so it cannot stand in for a real test.
- **All the wording lives in one `COPY` object** at the top of `lib/inquiryEmail.ts`, and both the HTML and text builders render from it — so the two versions cannot drift. Two conventions: `<em>` is the only tag allowed in `headline` (stripped for the text part), and `{link}` in `body` becomes an anchor in HTML and a bare URL on its own line in text.
- `esc()` is applied at the HTML call sites, never inside the shared helpers. `greeting()` in particular must return raw text: escaping it there renders `D'Angelo` as `D&#39;Angelo` in the plain-text part, which no client will decode.
- **`/dev/email-preview` renders the template in the browser with hot reload** — `?format=text` for the plain-text part, `?name=…` to try a greeting. It 404s whenever `NODE_ENV` is `production`, which covers Vercel Preview as well as Production. Use it instead of submitting the form: a real submission writes a live ClickUp task and makes you answer 34 questions to see one copy change. Check a name with an apostrophe: it must stay raw in the text part. Piece requests send no email, so there is nothing here to preview for them.
...
and a documented manual-action trigger. Add them when there are real client words. For the
same reason there is **no `award`**: the 2026 Stellar Awards on `/press` are builders'
awards, presented to Southern State Builders. The studio designed the home and the page
says so, but `award` names the recipient — a fact the code cannot re-derive, so don't
re-add it from the visible copy.
