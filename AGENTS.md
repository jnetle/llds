...
`/inquire` is the only route with a server round-trip, and `app/inquire/actions.ts` is the only `'use server'` file in the repo. A submission becomes **one ClickUp task with a PDF attached**, and then **one confirmation email to the visitor**. Needs `CLICKUP_API_TOKEN` (a `pk_…` personal token, sent bare in `Authorization` — no `Bearer`) and `CLICKUP_LIST_ID`; the email additionally needs `RESEND_API_KEY` and `INQUIRY_FROM_EMAIL`.
