...
The piece request form is the same shape with the PDF step removed — see `app/pieces/actions.ts`, which documents what it reuses (`lib/clickup.ts` unchanged, `allow()` under its own `piece-ip:` / `piece-to:` key prefixes) and why it skips the rest. Everything below describes the inquiry. Needs `CLICKUP_API_TOKEN` (a `pk_…` personal token, sent bare in `Authorization` — no `Bearer`) and `CLICKUP_LIST_ID`; the email additionally needs `RESEND_API_KEY` and `INQUIRY_FROM_EMAIL`.
...
- **`/dev/email-preview` renders both templates in the browser with hot reload** — `?format=text` for the plain-text part, `?name=…` to try a greeting, `?template=piece` for the piece request confirmation (plus `?piece=…&brand=…`). It 404s whenever `NODE_ENV` is `production`, which covers Vercel Preview as well as Production. Use it instead of submitting the form: a real submission writes a live ClickUp task and makes you answer 34 questions to see one copy change. Check a name with an apostrophe and a maker ending in `Co.` — the first must stay raw in the text part, and the second is why `{brand}` is never placed immediately before a full stop in `lib/pieceEmail.ts`.
