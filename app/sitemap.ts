...
/**
 /** Static routes in rough priority order. `/inquire` is included — it is the conversion page, and nothing about
  *  the form itself is private. */
...
/**
 /**
  * `updatedAt` is hand-typed and Next calls `toISOString()` on it, so an unparseable date fails static generation
  * with a bare `RangeError: Invalid time value` and no clue which record caused it. Fall back instead.
  */
